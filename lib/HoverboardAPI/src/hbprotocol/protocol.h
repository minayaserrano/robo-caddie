...
#pragma pack(                                                                  \
    push,                                                                      \
    4) // since on 'long' are used, alignment can be optimized for 4 bytes
...
typedef struct tag_PROTOCOL_MSG2 {
  ...
  unsigned char bytes[255]; // variable number of data bytes, with a checksum on
                            // the end, cmd is first
...
typedef struct tag_PROTOCOLCOUNT {
  ...
  unsigned long
      tx; // Count of sent messages (ACK, NACK and retries do not count)
  unsigned int txRetries; // how often were messages resend?
  unsigned int
      txFailed; // TX Messages which couldn't be deliveredr. No retries left.
...
typedef struct tag_PROTOCOL_STAT {
  ...
  int (*send_serial_data)(unsigned char *data,
                          int len); // Function Pointer to sending function
  int (*send_serial_data_wait)(unsigned char *data, int len);

  PROTOCOL_SUBSCRIBEDATA
  subscriptions[10]; // Subscriptions to periodic messages
...
typedef void (*PARAMSTAT_FN)(PROTOCOL_STAT *s, PARAMSTAT *param,
                             uint8_t fn_type, unsigned char *content, int len);

struct tag_PARAMSTAT {
  unsigned char code; // code in protocol to refer to this
  char *description;  // if non-null, description
  char *
      uistr; // if non-null, used in ascii protocol to adjust with f<str>num<cr>
...
// PUBLIC functions
/////////////////////////////////////////////////////////
extern void ascii_add_immediate(unsigned char letter,
                                int (*fn)(PROTOCOL_STAT *s, char byte,
                                          char *ascii_out),
                                char *description);
extern void ascii_add_line_fn(unsigned char letter,
                              int (*fn)(PROTOCOL_STAT *s, char *line,
                                        char *ascii_out),
                              char *description);
...
extern int setParamVariable(unsigned char code, char ui_type, void *ptr,
                            int len, char rw);
/////////////////////////////////////////////////////////////////
